...
//    }
    ...
    public void move(int ticksPerSecond) {
        ...
        pos = AEVector.plusVector_getVector(pos, AEVector.timesScalar_getVector(v, 1.0/ticksPerSecond));
    }
...
//BOUNCING BOUNCING BOUNCING BOUNCING BOUNCING BOUNCING BOUNCING BOUNCING
    ...
    public boolean bounceCircleOfCircle(MovingAnimationObject o, double restitution) {
        if(collision(this, o)) {
            ...
            double contactAngle = AE_UTIL.getAngle(getMidAsPoint(), o.getMidAsPoint()); //contact Angle ???????
           // double v1 = Math.sqrt(vX*vX + vY*vY);//scalar size of vX and vY from object 1
            //double v2 = Math.sqrt(o.vX*o.vX + o.vY*o.vY);//scalar size of vX and vY from object 2
                          double v1 = getV_X()*getV_Y()*Math.cos(mAngle1);//scalar size of vX and vY from object 1 ???????
                          double v2 = o.getV_X()*o.getV_Y()*Math.cos(mAngle2);//scalar size of vX and vY from object 2 ???????
                   //       double v = Math.sqrt
